...
def read_from_db(db_file):
    ...
    for line in db_file:
        ...
        if not line or line.startswith("#") or line.startswith('"#'):
            continue
        # Skip header rows that contain "IMSI" as a field name (case-insensitive)
        if "imsi" in line.lower() and ("name" in line.lower() or "key" in line.lower()):
            continue
        try:
            ...
            if len(fields) < 12:
                ...
                continue
            _name, imsi, key, op_type, op_c, amf, qci, ip_alloc, sst, sd, dnn, session_type = fields[:12]
