#!/bin/bash
# 4 UEs UL TCP: tmux window "srv" = host iperf3 -s :5201..5204, window "ue" = UE clients (UE→host, no -R).
...
echo "Windows: srv (4× iperf3 -s), ue (4× UE UL TCP clients, -b ${TCP_BW})  |  tmux session: $SESSION"
