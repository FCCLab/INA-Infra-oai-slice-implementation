#!/bin/bash
# 3 UEs DL UDP: tmux window "srv" = host iperf3 -s :5201..5203, window "ue" = UE clients iperf3 -u -R.
# -u UDP, -P 1, -b 1M per stream. -R reverses flow (server→UE).
...
echo "Windows: srv (3× iperf3 -s), ue (3× UE UDP clients)  |  tmux session: $SESSION"
