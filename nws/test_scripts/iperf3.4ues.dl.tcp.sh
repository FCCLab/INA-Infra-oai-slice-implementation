#!/bin/bash
# 4 UEs DL TCP: tmux window "srv" = host iperf3 -s :5201..5204, window "ue" = UE clients iperf3 -R.
# -R reverses flow (server→UE). Run from host shell (not inside containers).
...
for i in 0 1 2 3; do
  p="${ports[$i]}"
  tmux send-keys -t "$SESSION:srv.$i" "echo \"=== host iperf3 -s :${p} (DL TCP server) ===\"" C-m
...
for i in 0 1 2 3; do
  ue=$((i + 1))
  p="${ports[$i]}"
  tmux send-keys -t "$SESSION:ue.$i" "echo \"=== UE${ue} DL TCP -R -> ${HOST_IP} :${p} ===\"" C-m
  tmux send-keys -t "$SESSION:ue.$i" "docker exec -it nws-oai-nr-ue${ue} iperf3 -c ${HOST_IP} -R -t 0 -p ${p}" C-m
done

tmux select-window -t "$SESSION:srv"
iperf3_enable_mouse "$SESSION"
echo "Windows: srv (4× iperf3 -s), ue (4× UE TCP clients)  |  tmux session: $SESSION"
