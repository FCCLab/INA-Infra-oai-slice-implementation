#!/bin/bash
# 2×2 tmux: top-left host :5201 server, top-right host :5202, bottom UE1/UE2 iperf3 UDP UL clients (UE→host).
# -u UDP, -P 5 parallel streams, -b 100M per stream (~500 Mbit/s per UE). No -R (uplink).
...
tmux new-session -d -s "$SESSION" \; \
  ...
  send-keys 'echo "=== host iperf3 -s :5202 (UL UDP sink) ==="' C-m \; \
  ...
  send-keys "echo \"=== UE2 UL UDP -u -P 5 -b 100M -> ${HOST_IP} :5202 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue2 iperf3 -c ${HOST_IP} -u -t 0 -p 5202 -P 5 -b 100M" C-m
