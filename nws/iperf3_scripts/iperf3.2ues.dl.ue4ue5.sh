#!/bin/bash
# 2×2 tmux: top-left host :5201 server, top-right host :5202, bottom UE4/UE5 iperf3 -R (DL) clients.
# -R reverses flow (server→UE), same host IP/ports as UL script. Run from a normal shell.
...
tmux new-session -d -s "$SESSION" \; \
  ...
  send-keys "echo \"=== UE4 DL iperf3 -R -> ${HOST_IP} :5201 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue4 iperf3 -c ${HOST_IP} -R -t 0 -p 5201" C-m \; \
  select-pane -t "$SESSION:0.3" \; \
  send-keys "echo \"=== UE5 DL iperf3 -R -> ${HOST_IP} :5202 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue5 iperf3 -c ${HOST_IP} -R -t 0 -p 5202" C-m
