#!/bin/bash
# 2×2 tmux: top-left host :5201 server, top-right host :5202, bottom UE1/UE2 iperf3 -R (DL) clients.
...
tmux new-session -d -s "$SESSION" \; \
  ...
  send-keys "echo \"=== UE2 DL iperf3 -R -> ${HOST_IP} :5202 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue2 iperf3 -c ${HOST_IP} -R -t 0 -p 5202" C-m
