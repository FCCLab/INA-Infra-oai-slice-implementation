#!/bin/bash
# 4 UEs DL: tmux window "srv" = host iperf3 -s :5201..5204, window "ue" = UE clients iperf3 -R.
...
for i in 0 1 2 3; do
  p="${ports[$i]}"
  tmux send-keys -t "$SESSION:srv.$i" "echo \"=== host iperf3 -s :${p} (DL server) ===\"" C-m
...
for i in 0 1 2 3; do
  ue=$((i + 1))
  p="${ports[$i]}"
  tmux send-keys -t "$SESSION:ue.$i" "echo \"=== UE${ue} DL iperf3 -R -> ${HOST_IP} :${p} ===\"" C-m
...
echo "Windows: srv (4× iperf3 -s), ue (4× UE clients)  |  tmux session: $SESSION"
