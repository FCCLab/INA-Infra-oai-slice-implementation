#!/bin/bash
# 2×2 tmux: same as iperf3.2ues.ul.sh but each UE client uses -b 1M (1 Mbit/s cap, iperf3 --bitrate).
# UE → host uplink. Override: IPERF_BITRATE=2M ./iperf3.2ues.ul.1M.sh
...
tmux new-session -d -s "$SESSION" \; \
  ...
  send-keys 'echo "=== host iperf3 -s :5201 (UL server) ==="' C-m \; \
  send-keys 'iperf3 -s -p 5201' C-m \; \
  select-pane -t "$SESSION:0.1" \; \
  send-keys 'echo "=== host iperf3 -s :5202 (UL server) ==="' C-m \; \
  send-keys 'iperf3 -s -p 5202' C-m \; \
  select-pane -t "$SESSION:0.2" \; \
  send-keys "echo \"=== UE1 UL iperf3 -b ${IPERF_BITRATE} -> ${HOST_IP} :5201 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue1 iperf3 -c ${HOST_IP} -b ${IPERF_BITRATE} -t 0 -p 5201" C-m \; \
  select-pane -t "$SESSION:0.3" \; \
  send-keys "echo \"=== UE2 UL iperf3 -b ${IPERF_BITRATE} -> ${HOST_IP} :5202 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue2 iperf3 -c ${HOST_IP} -b ${IPERF_BITRATE} -t 0 -p 5202" C-m
