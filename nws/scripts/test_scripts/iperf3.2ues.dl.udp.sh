#!/bin/bash
# 2×2 tmux: top-left host :5201 server, top-right host :5202, bottom UE1/UE2 iperf3 UDP -R (DL) clients.
# -u UDP, -P 5 parallel streams, -b 100M per stream (~500 Mbit/s per UE). -R reverses flow (server→UE).
...
tmux new-session -d -s "$SESSION" \; \
  ...
  send-keys "echo \"=== UE2 DL UDP -R -u -P 5 -b 100M -> ${HOST_IP} :5202 ===\"" C-m \; \
  send-keys "docker exec -it nws-oai-nr-ue2 iperf3 -c ${HOST_IP} -u -R -t 0 -p 5202 -P 5 -b 100M" C-m
