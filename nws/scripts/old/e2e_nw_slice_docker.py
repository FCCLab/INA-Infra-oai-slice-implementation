...
DEFAULT_RAN_COMPOSE = NWS_DIR / "docker-compose" / "docker-compose.open5gs.5slices.nsul.yaml"
DEFAULT_GNB_YAML = NWS_DIR / "configs" / "gnb" / "gnb.sa.band78.106prb.rfsim.open5gs.5slices.nsul.yaml"
...
def rebuild_gnb_binary(compose_file: Path, *, cwd: Path, log: logging.Logger) -> bool:
    """
    Bypassed: Release/production images are immutable and have baked-in binaries.
    """
    log.info("Bypassing gNB + UE binary rebuild (running native release images directly).")
    return True
