...
CONFIGFILE=/workspace/cu-up.yaml
BUILD_DIR=/workspace/openairinterface5g/cmake_targets/ran_build/build

if [ ! -f "$CONFIGFILE" ]; then
  echo "Error: CU-UP configuration file $CONFIGFILE not found"
  exit 1
fi
...
if [ ! -f "./nr-cuup" ]; then
  echo "Error: nr-cuup not found in $BUILD_DIR. Please build first using ./build.sh"
  exit 1
fi

args=("./nr-cuup" "-O" "$CONFIGFILE")
...
LOG_FILE="${CUUP_LOG_FILE:-/workspace/log/cu-up.log}"
