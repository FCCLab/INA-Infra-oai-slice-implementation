#!/usr/bin/env python3
"""
3GPP Network Slicing Live rfsim Automated Test Suite (PR #451)

Features:
  1. Auto-preparation with step-by-step progress ticks:
     - [1] Check 5GC (Open5GS), bring up if not running [✓]
     - [2] Check/Run OAI gNB (rfsim) [✓]
     - [3] Check/Run UEs and verify PDU IP assignment [✓]
     - [4] Apply Slice Policy via E2 / FlexRIC xApp REST API [✓]
     - [5] Start Traffic in interactive tiled Tmux panes [✓]
  2. Live Sampling Countdown (e.g. 30s -> 0s) [✓]
  3. Step-by-step Evaluation:
     - 5.1 Evaluate active slice bandwidth / throughput -> [OK]
     - 5.2 Evaluate idle slice PRB reservation / sharing -> [OK]
  4. Final Verdict: PASSED / FAILED [✓]
  5. Dedicated Timestamped Log Storage under `nws/test/logs/`
"""
...
def auto_prepare_testbed(num_ues: int = 5, scenario_name: Optional[str] = None, force_restart: bool = False) -> bool:
    ...
    print_step(1, 4, "Checking Open5GS Core (nws-5gc)", done=False)
    if not is_container_running(CORE_CONTAINER):
        ...
        if ret != 0:
            ...
            return False
        time.sleep(2)
    print_step(1, 4, "Checking Open5GS Core (nws-5gc)", done=True, msg="Running & Healthy (Reused)")

    base_group = get_scenario_base_group(scenario_name or "as_no_slice")
    last_group = STATE_FILE.read_text().strip() if STATE_FILE.is_file() else ""
    ...
    if not force_restart and ran_healthy and all_ues_healthy and has_active_pdus and (last_group == base_group):
        print_step(2, 4, f"Checking OAI gNB ({GNB_CONTAINER})", done=True, msg=f"Running (Reused - Same config '{base_group}')")
        print_step(
            3,
            4,
        ...
        print_step(4, 4, "5G End-to-End Testbed Readiness", done=True, msg="Ready for Slicing Tests (No Restart Needed)")
        print("========================================================================\n")
        return True

    # 2. Fresh RAN & UE Bringup via bringup.py if config changed or containers down
    is_pf = base_group.startswith("pf") or base_group in ("pf_only", "pf")
    sch_type = "PF" if is_pf else "NSBOTH"
    print_step(2, 4, f"Configuring {'RAN Baseline (sch=PF)' if is_pf else 'Slicing Baseline & Bringup'} ({base_group})", done=False)
    bringup_cmd = [
        sys.executable,
        str(BRINGUP_PY),
        "--ues", str(num_ues),
    ...
    print_step(4, 4, "5G End-to-End Testbed Readiness", done=True, msg="Ready for Slicing Tests")
    print("========================================================================\n")
    return True
...
def apply_xapp_policy(rules: list[SliceRule], log_dir: Path):
    """Apply slice policy via FlexRIC xApp REST API and log request."""
    if not rules:
        return True
    ...
    try:
        with urllib.request.urlopen(req, timeout=3) as resp:
            if resp.status == 200:
                print("  -> Applied slice policy via FlexRIC xApp REST API.")
                time.sleep(1.5)
                return True
    except Exception as e:
        print(f"  -> FlexRIC xApp API not available ({e}). Policy will use current gNB configuration.")
        return False
...
def launch_tmux_session(sc: TestScenario, direction: str, duration: int, attach: bool = False, no_attach: bool = False):
    ...
    # Step 1: Ensure FlexRIC policy is active if running
    apply_xapp_policy(sc.rules, log_dir)
...
def display_interactive_menu():
    ...
    while True:
        ...
        run_batch_tests(tests_to_run, duration=30, skip_prep=False, force_restart=False)
...
def main():
    ...
    if "TMUX" not in os.environ and sys.stdin.isatty() and not ("--no-attach" in sys.argv):
    ...
    parser = argparse.ArgumentParser(description="Live 3GPP 5G rfsim Slicing Test Matrix with Auto-Preparation and Evaluation (PR #451)")
    parser.add_argument(
        "--test",
        nargs="+",
        default=["as_no_slice"],
    ...
    parser.add_argument("--force-restart", action="store_true", help="Force restart gNB and UEs even if same slice config is running")
