...
Compose mounts `config/flexric.conf` as `/workspace/flexric.conf` in `nws-nearRT-RIC`.
...
cd nws
./build_flexric.sh
source config/flexric.connection.env
...
- `e2_agent.near_ric_ip_addr` in gNB YAML = `NEAR_RIC_IP` in `config/flexric.conf` (default `192.168.201.142`)
...
1. Edit `NEAR_RIC_IP` in `config/flexric.conf`
